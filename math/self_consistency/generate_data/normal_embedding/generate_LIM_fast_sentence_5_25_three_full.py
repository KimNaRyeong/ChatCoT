...
model_for_func_name = fasttext.load_model('cc.en.300.bin')
model_for_func_arg = fasttext.load_model('cc.en.300.bin')
...
if embedding_size != 300:
    ...
    fasttext.util.reduce_model(model_for_func_name, 5)
    fasttext.util.reduce_model(model_for_func_arg, 25)
    ...
    for reasoning_steps in trans_reasoning_paths:
        ...
        for rs in reasoning_steps:
            if "function_call" in rs:
                # print(rs) #{'function_call': 'Do_not_use_tool', 'arguments': None}
                func_emb = embed_reasoning_path(str(rs["function_call"]), False, "func_name")
                arg_emb = embed_reasoning_path(str(rs["arguments"]), False, "func_arg")
                fc_emb =  np.concatenate((func_emb, arg_emb), axis = 0)
                embed_reasoning_steps.append(fc_emb)
            else:
                if "answer" in rs:
                    answer_emb = embed_reasoning_path(str(rs["answer"]), False, "answer")
                else:
                    answer_emb = embed_reasoning_path(rs, False, "answer")
...
def embed_reasoning_path(rs, word_vector, rs_type):
    if rs_type == "func_name":
        model = model_for_func_name
    elif rs_type == "func_arg":
        model = model_for_func_arg
    else:
...
def main():
    ...
    for k in ks_for_fa:
        ...
        if not os.path.exists(f"/workspace/LIM_data/normal_embedding/three_length/concat/5_25/{k}"):
            os.makedirs(f"/workspace/LIM_data/normal_embedding/three_length/concat/5_25/{k}")
        
        torch.save({
            "dataset_FA": dataset_for_k_FA,
            "y": dataset_for_k_y
        }, f"/workspace/LIM_data/normal_embedding/three_length/concat/5_25/{k}/lstm_fa_dataset_full.pth")
