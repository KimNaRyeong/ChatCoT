...
if embedding_size != 300:
    ...
    fasttext.util.reduce_model(model_for_func_arg, 95)
    ...
    for node in LIG.nodes():
        ...
        #     node_text.append(node_dict['function_call'])
            ...
            if 'answer' in node_dict.keys():
                embed_nodes.append(embed_reasoning_path(node_dict['answer'], True, 'answer'))
            else:
                func_emb = embed_reasoning_path(str(node_dict['function_call']), True, 'func_name')
                arg_emb = embed_reasoning_path(str(node_dict['arguments']), True, 'func_arg')
...
def main():
    ...
    for k in ks_for_fa:
        ...
        if not os.path.exists(f"/workspace/LIG_data/normal_embedding/three_length/concat/5_95/{k}"):
            os.makedirs(f"/workspace/LIG_data/normal_embedding/three_length/concat/5_95/{k}")
        
        torch.save({
            "dataset_FA": dataset_for_k_FA,
        }, f"/workspace/LIG_data/normal_embedding/three_length/concat/5_95/{k}/gcn_fa_dataset_full.pth")
