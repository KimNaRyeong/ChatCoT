...
def generate_one_LIG(reasoning_paths, whole_arg_list, label, k):
    ...
    for node in LIG.nodes():
        ...
        ones_vector = torch.ones(2, dtype = torch.float)
...
def main():
    ...
    for k in ks:
        ...
        if not os.path.exists(f"/workspace/LIG_data/two_length/{k}"):
            os.makedirs(f"/workspace/LIG_data/two_length/{k}")
        
        torch.save({
            "dataset_S": dataset_for_k_S,
            "dataset_F": dataset_for_k_F,
            "dataset_FA": dataset_for_k_FA,
        }, f"/workspace/LIG_data/two_length/{k}/gcn_dataset_short.pth")
