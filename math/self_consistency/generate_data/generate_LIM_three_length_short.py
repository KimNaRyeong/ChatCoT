...
def generate_one_LIM(reasoning_paths, whole_arg_list, k):
    ...
    for rp in reasoning_paths:
        ...
        while len(F_path) < int(k):
            ...
            FA_path.append(torch.zeros(23, dtype=torch.float))
...
def main():
    ...
    for k in ks:
        ...
        if not os.path.exists(f"/workspace/LIM_data/three_length/{k}"):
            os.makedirs(f"/workspace/LIM_data/three_length/{k}")
        
        torch.save({
            "dataset_F": dataset_for_k_F,
            "dataset_FA": dataset_for_k_FA,
            "y": dataset_for_k_y
        }, f"/workspace/LIM_data/three_length/{k}/lstm_dataset_short.pth")
